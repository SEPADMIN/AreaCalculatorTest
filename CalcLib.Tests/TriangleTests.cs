...
namespace CalcLibTest {
    public class TriangleTests {
        ...
        public void RegularPolygonImplicitCastTests() {
            ...
            Assert.AreEqual(t.GetSidesForTests()[0], rp.GetSideLength());
        ...
        public void RegularPolygonExplicitCastTests() {
            ...
            Assert.DoesNotThrow(() => { Triangle t = (Triangle)rp; });

            Triangle t = (Triangle)rp;
            foreach(double side in t.GetSidesForTests()) {
                Assert.AreEqual(5, side);
            }

            RegularPolygon rp2 = new(4, 1);
            Assert.Throws<InvalidCastException>(() => { Triangle t = (Triangle)rp2; });
        }
        ...
        public void FindAreaTests() {
            ...
            Assert.IsTrue(resultResidual < t.GetComparisonToleranceForTests());

            Triangle t2 = new(3, 4, 5);
            double result2 = t2.FindArea();
            double expectedResult2 = 6;
            double resultResidual2 = Math.Abs(expectedResult2 - result2);
            Assert.IsTrue(resultResidual2 < t2.GetComparisonToleranceForTests());
            ...
            Assert.IsTrue(resultResidual3 < t3.GetComparisonToleranceForTests());
        }
